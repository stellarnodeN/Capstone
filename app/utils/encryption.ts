...
// Encrypted mock storage for development/testing
const mockStorage = new Map<string, { encrypted: string, nonce: string, ephemeralPublicKey: string }>();
...
export class EncryptionManager {
    ...
    async encryptAndStore(
        ...
        if (!researcherPublicKey || researcherPublicKey.length !== 32) {
            throw new Error('Invalid public key: must be 32 bytes');
        }
        
        // Validate data size (max 1MB to prevent DoS)
        const dataString = JSON.stringify(data);
        if (dataString.length > 1024 * 1024) {
            throw new Error('Data too large: maximum 1MB allowed');
        }
        ...
        const messageUint8 = new TextEncoder().encode(dataString);
        ...
        if (config.provider === 'custom' && !config.apiKey) {
            ...
            // Store encrypted data in mock storage (not plaintext)
        ...
        for (let attempt = 1; attempt <= (config.retries || 3); attempt++) {
            ...
            console.error(`IPFS upload attempt ${attempt} failed:`, {
                attempt,
...
export class DecryptionManager {
    ...
    async fetchAndDecrypt(
        ...
        // For development/testing, use mock IPFS if no API key is available
        if (config.provider === 'custom' && !config.apiKey) {
            // Mock decryption for development - retrieve encrypted data from mock storage
            const storedData = mockStorage.get(cid);
            if (storedData) {
                // Decrypt the stored data using the provided private key
                const encryptedData = decodeBase64(storedData.encrypted);
                const nonce = decodeBase64(storedData.nonce);
                const ephemeralPublicKey = decodeBase64(storedData.ephemeralPublicKey);
